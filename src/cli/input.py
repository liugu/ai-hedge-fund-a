...
def add_common_args(
    ...
    *,
    ...
    parser.add_argument(
        ...
        help="逗号分隔的股票代码列表（例如：AAPL,MSFT,GOOGL 或 600519.SH,000001.SZ）",
    )
    if include_analyst_flags:
        parser.add_argument(
            ...
            type=str,
            required=False,
            help="逗号分隔的分析师列表（例如：michael_burry,other_analyst）",
        )
        parser.add_argument(
            "--analysts-all",
            action="store_true",
            help="使用所有可用分析师（覆盖--analysts参数）",
        )
    if include_ollama:
        parser.add_argument("--ollama", action="store_true", help="使用Ollama进行本地LLM推理")
...
def add_date_args(parser: argparse.ArgumentParser, *, default_months_back: int | None = None) -> argparse.ArgumentParser:
    if default_months_back is None:
        parser.add_argument("--start-date", type=str, help="开始日期（YYYY-MM-DD格式）")
        parser.add_argument("--end-date", type=str, help="结束日期（YYYY-MM-DD格式）")
    else:
        parser.add_argument(
            "--end-date",
            type=str,
...
def select_model(use_ollama: bool, model_flag: str | None = None) -> tuple[str, str]:
    ...
    if use_ollama:
        print(f"{Fore.CYAN}使用Ollama进行本地LLM推理。{Style.RESET_ALL}")
        ...
        if not ensure_ollama_and_model(model_name):
            print(f"{Fore.RED}无法继续，需要Ollama和所选模型。{Style.RESET_ALL}")
            sys.exit(1)
...
def parse_cli_inputs(
    *,
    ...
    parser.add_argument(
        ...
        type=float,
        default=100000.0,
        help="初始现金头寸（别名：--initial-capital）。默认值：100000.0",
    )
    parser.add_argument(
        "--margin-requirement",
